...
def retire_sold(tabs):
    ...
    for tab in tabs:
        for p in list(tab):
            if p["address"] not in SOLD_ADDRS:
                continue
            prev = found.get(p["address"])
            _coc = p.get("fin", {}).get("coc")
            _pcoc = prev.get("fin", {}).get("coc") if prev else None
            if prev is None or (_coc is not None and (_pcoc is None or _coc > _pcoc)):
                found[p["address"]] = p
            tab.remove(p)
